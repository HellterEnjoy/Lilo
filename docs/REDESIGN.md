...
| Editor | Existing Markdown editing, tasks, formatting, wiki links, aliases, attachments, inline images and outline navigation remain connected. Readable width and text size are configurable. |
...
- `src/app/navigation.rs`: workspace header, responsive panel toggles and shared navigation cache.
- `src/app/editor.rs`: editor integration.
- `src/app/settings.rs`: categorized settings and template actions.
- `src/app/actions.rs`: the shared `CommandAction` dispatcher used by menus, shortcuts, the palette and compact controls.
- `src/app/shell.rs`, `explorer.rs`, `inspector.rs`: root layout and the two current side panels.
- `src/app/notes.rs`, `persistence.rs`, `recovery.rs`, `vault.rs`: note operations, autosave/watcher coordination, recovery and vault switching.
...
- `src/app.rs`: application state, construction, `eframe::App` and root module coordination.
- `src/storage.rs` and `src/storage/`: a stable facade over settings, vault, note, recovery, transfer, diagnostics and safe-I/O modules.

## Validation

- Release validation covers formatting, tests, Clippy with warnings denied and an executable build.
